...
getPrediction <- function(p) {
  ...
  if(bCount>1) {
    ...
    for(i in 1:bCount){
      ...
      knotDayNum[i] <- zeroDayNum + betaChangeIntervalDays*i - (betaChangeIntervalDays/2) # middle of the week # middle of the week
    }
  ...
  #print(odeParameters)
  #print(odeZeroState)
  #print(paste0("beta at start day: ",get_beta(zeroDayNum)))
...
pretrainedPar[["b1"]] <- pretrainedPar[["beta"]]
pretrainedPar <- pretrainedPar[names(pretrainedPar) != "beta"]
print(pretrainedPar)

#print(optRes)
print(paste0("First day: ",pretrainedPar[["zeroDayNum"]]))
print(paste0("Beta: ",nonNegativeParam(pretrainedPar[["b1"]])))
print(paste0("Gamma: ",sigmoid(pretrainedPar[["gamma"]])))
...
bRaw <- unlist(optFineRes$par[b1_idx:(length(optFineRes$par))])
names(bRaw) <- NULL
...
paramsList <- list()
paramsList$RawGamma <- optFineRes$par[["gamma"]]
paramsList$Gamma <- gamma
paramsList$bRaw <- bRaw
...
paramsList$RawFirstDayInfectedCount <- optFineRes$par[["firstDayInfectedCount"]]
