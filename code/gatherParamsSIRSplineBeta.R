...
outRecentFile <- args[2]
outWeeklyFile <- args[3]
...
for(pars_df_path in inFiles) {
  ...
  weeks <- length(json_data[["WeeklyBeta"]])
  weekNums <- 1:weeks
  weeklyData <- data.frame(
    Province = rep(province, weeks),
    Country = rep(country, weeks),
    WeekNum = weekNums,
    Date = start_date + as.integer(json_data[["FirstDayNum"]]) - 1 + (weekNums*7) - 4,
    Beta = json_data[["WeeklyBeta"]]
    )
  weeklyData$TransmissionInterval <- 1.0 / weeklyData$Beta
  weeklyData$R0 <- weeklyData$Beta / json_data[["Gamma"]]
  
  if(is.null(accWeelky))
    accWeelky <- weeklyData
  else
    accWeelky <- rbind(accWeelky, weeklyData)
...
write.csv(acc,file=outRecentFile,row.names = F, quote = T)
write.csv(accWeelky,file=outWeeklyFile, row.names = F,quote = T)