...
for(pars_df_path in inFiles) {
  ...
  zeroThreshold <- 5e-3
  # Qualicy Control
  if(pars_df$Beta < zeroThreshold || pars_df$Sigma < zeroThreshold || pars_df$Gamma < zeroThreshold) {
    print(paste0("Skipping '",province,"' country '",country,"' due to too near zero rate parameters (QC)"))
    next;
  }
  ...
  pars_df$PeakInfectedDate <- as.character(start_date+pars_df$PeakInfectedDayNum-1)
  pars_df$PeakExposedDate <- as.character(start_date+pars_df$PeakExposedDayNum-1)
  pars_df <- pars_df[,c(origColsN+1,origColsN+2,1:origColsN,(origColsN+3):(origColsN+5))]
  if(is.null(acc))
    acc <- pars_df
  else
