...
getPrediction <- function(p) {
  ...
  firstDayExposedCount = abs(p[["firstDayExposedCount"]])
  ...
  zeroDayNum <- round(zeroDayNum,1)
  
  odeDays <- seq(zeroDayNum, zeroDayNum+366, by = 0.1)
  odeParameters <- c(beta = beta, gamma=gamma, sigma=sigma, N=N)
  
  odeZeroState <- c(S=N, E=firstDayExposedCount,I = firstDayInfectedCount,R=0)
  
  out <- as.data.frame(ode(y = odeZeroState, times = odeDays, func = SEIR, parms = odeParameters))
...
getPredRunTable<-function(p) {
  ...
  pop <- sigmoid(p[["popFactor"]])*popCount
  if(pop > maxConfirmed)
    pop <- maxConfirmed
  ...
  if(sum(is.na(m1$exposed.pred) & m1$days<=erliestOut$days)>0)
    m1[is.na(m1$exposed.pred) & m1$days<=erliestOut$days,]$exposed.pred <- 0
...
for(i in (1:optIters)) {
  ...
  startP = c(zeroDayNum=firstDayIdx, # when the infection started
                firstDayExposedCount=runif(1,min=1,max=10), # how many exposed on the first day
             firstDayInfectedCount=runif(1,min=1,max=10), # how many infected on the first day
             beta = runif(1), # beta
             sigma =runif(1), # sigma
             gamma = runif(1),# gamma
             popFactor = 1  # pop factor
             ) 
  #print(startP)
...
firstDayExposedCount = abs(optRes$par[["firstDayExposedCount"]])
...
max_val <- max(c(obsSource$Infected,obsSource$Removed,obsSource$Exposed)) * 1.1
